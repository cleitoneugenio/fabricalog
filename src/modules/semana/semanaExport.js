...
export async function exportSemana(semana) {
  ...
  const allRows = [
    ...
    ['Enforna 1', ...semana.dias.map(d => { const f = (d.enfornas ?? [d.enforna ?? ''])[0] ?? ''; const dp = f && d.enfornasDupla?.[f]; return dp ? `${f} (${dp})` : f; }), ''],
    ['Enforna 2', ...semana.dias.map(d => { const f = (d.enfornas ?? [])[1] ?? ''; const dp = f && d.enfornasDupla?.[f]; return dp ? `${f} (${dp})` : f; }), ''],
    ['Enforna 3', ...semana.dias.map(d => { const f = (d.enfornas ?? [])[2] ?? ''; const dp = f && d.enfornasDupla?.[f]; return dp ? `${f} (${dp})` : f; }), ''],
